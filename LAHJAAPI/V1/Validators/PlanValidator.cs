...
namespace LAHJAAPI.V1.Validators
{
    ...
    public class PlanValidator : BaseValidator<PlanRequestDso, PlanValidatorStates>, ITValidator
    {
        private readonly IConditionChecker _checker;
        ...
        protected override void InitializeConditions()
        {
            ...
            _provider.Register(PlanValidatorStates.HasProductId,
            new LambdaCondition<PlanRequestDso>(
                nameof(PlanValidatorStates.HasProductId),
                p => !string.IsNullOrWhiteSpace(p.ProductId),
                "Product ID is required"
            )
        );

            _provider.Register(PlanValidatorStates.HasProductName,
          new LambdaCondition<PlanResponseFilterDso>(
              nameof(PlanValidatorStates.HasProductName),
              p => IsNamePlan(p),
              "Product ID is required"
          )
      );
            ...
            _provider.Register(PlanValidatorStates.HasValidBillingPeriod,
                new LambdaCondition<PlanRequestDso>(
                    nameof(PlanValidatorStates.HasValidBillingPeriod),
                    p => new[] { "daily", "weekly", "monthly", "yearly" }.Contains(p.BillingPeriod?.ToLower()),
                    "Billing period must be daily, weekly, monthly, or yearly"
                )
            );

            _provider.Register(PlanValidatorStates.HasPositiveAmount,
                new LambdaCondition<PlanRequestDso>(
                    nameof(PlanValidatorStates.HasPositiveAmount),
                    p => p.Amount > 0,
                    "Amount must be greater than zero"
                )
            );

            _provider.Register(PlanValidatorStates.HasImages,
                new LambdaCondition<PlanRequestDso>(
                    nameof(PlanValidatorStates.HasImages),
                    p => p.Images != null && p.Images.Any(),
            ...
            _provider.Register(PlanValidatorStates.IsActivePlan,
                new LambdaCondition<PlanResponseFilterDso>(
                    nameof(PlanValidatorStates.IsActivePlan),
                    p =>IPlanActive(p),
                    "Plan must be active"
                )
            );
            ...
            _provider.Register(PlanValidatorStates.IsFeturePlan,
             new LambdaCondition<PlanFeatureResponseFilterDso>(
                 nameof(PlanValidatorStates.IsFeturePlan),
                 p => IsFeaturePlan(p),
                 "Plan must be active"
             )
         );
        ...
        private async Task<bool>IPlanActive(PlanResponseFilterDso context)
        {
            var result = _checker.Injector.Context.Plans
                .FirstOrDefault(x => x.Id == context.Id &&
                context.Active==true);
            return result != null;
        }


        private async Task<bool>IsNamePlan(PlanResponseFilterDso context)
        {
            var result = _checker.Injector.Context.Plans
                .FirstOrDefault(x => x.Id == context.Id &&
                string.Equals(context.ProductName, x.ProductName, StringComparison.OrdinalIgnoreCase));
                return result != null;
        }


        private async Task<bool> IsFeaturePlan(PlanFeatureResponseFilterDso context)
        {

            var result =  _checker.Injector.Context.PlanFeatures
                .FirstOrDefault(x => x.PlanId == context.PlanId &&
                                          string.Equals(context.Name, x.Name, StringComparison.OrdinalIgnoreCase) &&
                                          string.Equals(context.Description, x.Description, StringComparison.OrdinalIgnoreCase));
